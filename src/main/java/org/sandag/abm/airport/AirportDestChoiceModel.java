...
public class AirportDestChoiceModel
{
    ...
    private double[][][]                mgraProbabilities;                        // by
                                                                                   ...
                                                                                   // (sequential,
                                                                                   // 0-based)
    private double[][]                  tazProbabilities;                         // by
                                                                                   // segment,
                                                                                   // origin
                                                                                   // taz
                                                                                   // 0-based
    ...
    public AirportDestChoiceModel(HashMap<String, String> rbMap, AirportDmuFactoryIf dmuFactory, String airportCode)
    {
        ...
        for (int i = 0; i < AirportModelStructure.INTERNAL_PURPOSES; ++i)
        {
            ...
            // page and data page.
            destModel[i] = new ChoiceModelApplication(airportDistUecFileName, destModelPage,
        ...
        sizeTermUEC = new UtilityExpressionCalculator(new File(airportDistUecFileName), sizePage,
                dataPage, rbMap, dmuFactory.getAirportModelDMU());
    ...
    public void calculateMgraProbabilities(AirportDmuFactoryIf dmuFactory)
    {
        ...
        for (int segment = 0; segment < alternatives; ++segment)
        {
            for (int taz = 0; taz < tazs.length; ++taz)
            {
                ...
                if (sum > 0.0) sizeTerms[segment][tazNumber] = Math.log(sum + 1.0);
                ...
                if (sum > 0.0 && Math.abs(lastProb - 1.0) > 0.000001)
                    logger.info("Error: segment " + segment + " taz " + tazNumber
    ...
    public void calculateZipCodes()
    {
        ...
        String fileName = directory + Util.getStringValueFromPropertyMap(rbMap, "mgra.socec.file");
    ...
    public void calculateTazProbabilities(AirportDmuFactoryIf dmuFactory)
    {
        ...
        for (int segment = 0; segment < segments; ++segment)
        {
            ...
            int purpose = AirportModelStructure.getPurposeFromDCSizeSegment(segment);
            int income = AirportModelStructure.getIncomeFromDCSizeSegment(segment);
